...
function draw(open,glance=0,happy=0){ctx.clearRect(0,0,2000,2000);if(open>=.999&&!glance&&!happy)return;for(const [x,y,color] of [[886,1123,68],[1100,1125,73]]){const g=ctx.createLinearGradient(x-63,0,x+63,0);g.addColorStop(0,`rgb(${color-1},${color-1},${color-1})`);g.addColorStop(1,`rgb(${color+1},${color+1},${color+1})`);ctx.fillStyle=g;ctx.beginPath();ctx.ellipse(x,y,62,61,0,0,Math.PI*2);ctx.fill();ctx.fillStyle='#fff';ctx.strokeStyle='#fff';if(happy){ctx.lineWidth=13;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x-38,y+8);ctx.quadraticCurveTo(x,y-46,x+38,y+8);ctx.stroke()}else{ctx.beginPath();ctx.ellipse(x+glance,y,55.5-Math.abs(glance)*.5,Math.max(3,53.5*open),0,0,Math.PI*2);ctx.fill()}}}
let timer,step=0;
function animate(){if(reduced.matches||document.hidden){draw(1);timer=setTimeout(animate,1000);return}const mode=step++%4,t=performance.now(),duration=mode===0||mode===2?230:1100;function frame(now){if(reduced.matches||document.hidden){draw(1);timer=setTimeout(animate,1000);return}const p=(now-t)/duration;if(p>=1){draw(1);timer=setTimeout(animate,mode===0?1100:1700);return}if(mode===0||mode===2)draw(Math.abs(2*p-1));else if(mode===1)draw(1,14*Math.sin(p*Math.PI));else draw(1,0,1);requestAnimationFrame(frame)}requestAnimationFrame(frame)}
function start(){clearTimeout(timer);timer=setTimeout(animate,450)}if(img.complete)start();else img.addEventListener('load',start,{once:true});reduced.addEventListener('change',()=>{draw(1)});
})();
