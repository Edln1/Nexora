...
function draw(open=1){ctx.clearRect(0,0,2000,2000);if(open>=.999)return;for(const [x,y,color] of [[886,1123,68],[1100,1125,73]]){const g=ctx.createLinearGradient(x-63,0,x+63,0);g.addColorStop(0,`rgb(${color-1},${color-1},${color-1})`);g.addColorStop(1,`rgb(${color+1},${color+1},${color+1})`);ctx.fillStyle=g;ctx.beginPath();ctx.ellipse(x,y,62,61,0,0,Math.PI*2);ctx.fill();ctx.fillStyle='#fff';ctx.beginPath();ctx.ellipse(x,y,55.5,Math.max(3,53.5*open),0,0,Math.PI*2);ctx.fill()}}
let timer;
function animate(){if(reduced.matches||document.hidden){draw();timer=setTimeout(animate,1000);return}const t=performance.now();function frame(now){if(reduced.matches||document.hidden){draw();timer=setTimeout(animate,1000);return}const p=(now-t)/230;if(p>=1){draw();timer=setTimeout(animate,2400);return}draw(Math.abs(2*p-1));requestAnimationFrame(frame)}requestAnimationFrame(frame)}
